...
## LN1
refit(obj_list[[2]], samplename=samples[2], sex=sex, ploidy=2, purity=0.55, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## LN5 
refit(obj_list[[3]], samplename=samples[3], sex=sex, ploidy=2, purity=0.55, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## N1
refit(obj_list[[4]], samplename=samples[4], sex=sex, ploidy=2, purity=1, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## PT6
refit(obj_list[[5]], samplename=samples[5], sex=sex, ploidy=2, purity=0.57, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## PT1
refit(obj_list[[6]], samplename=samples[6], sex=sex, ploidy=2, purity=0.37, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## PT2
refit(obj_list[[7]], samplename=samples[7], sex=sex, ploidy=2, purity=0.37, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## PT4
refit(obj_list[[8]], samplename=samples[8], sex=sex, ploidy=2, purity=0.47, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## PT5
refit(obj_list[[9]], samplename=samples[9], sex=sex, ploidy=2, purity=0.49, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))

## TD1
refit(obj_list[[10]], samplename=samples[10], sex=sex, ploidy=2, purity=0.57, save=T, output_dir=here(paste0('processed_data/copynumber/',patient)))
...
tree_scna <- ape::rotate(tree_scna, 12)
tree_scna <- ape::rotate(tree_scna, 18)
tree_scna <- ape::rotate(tree_scna, 15)
tree_scna <- ape::rotate(tree_scna, 16)
#plot(tree_scna, type='u'); nodelabels()
...
scna_tree <- root(scna_tree,outgroup=grep(paste0('^N'),scna_tree$tip.label),resolve.root=TRUE) 
scna_tree <- addConfidences(scna_tree, bs_info$bstrees)
scna_tree$node.label <- round(100*scna_tree$node.label)
scna_tree <- ape::rotate(scna_tree, 12)
scna_tree <- ape::rotate(scna_tree, 15)
p_bsscna_tree <- plot_bootstrapped_tree_unrooted(scna_tree, groups, paste0(patient,' SCNA'))
p <- plot_grid(p_bsscna_tree, p_bstree_ad_conf, res$plot, nrow=1)
